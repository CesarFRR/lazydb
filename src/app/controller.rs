...
impl App {
    ...
    fn open_row_inspector(&mut self) {
        ...
        // Formato vertical: ▸ columna, luego valor en línea(s) aparte.
        // El word-wrap lo maneja el Paragraph del modal automáticamente.
        self.row_inspector_lines = columns
            .iter()
            .zip(values.iter().chain(std::iter::repeat(&"")))
            .flat_map(|(col, val)| vec![format!("▸ {col}"), format!("  {val}"), String::new()])
            .collect();
